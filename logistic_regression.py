...
def do_experiments(start, end, step_num):
    ...
    for i, distance in enumerate(shift_distances, 1):
        ...
        plt.scatter(X[y == 0, 0], X[y == 0, 1], color='blue', label='Class 0' if i == 1 else "")
        plt.scatter(X[y == 1, 0], X[y == 1, 1], color='red', label='Class 1' if i == 1 else "")
        ...
        plt.plot(x_values, y_values, color='green', linestyle='--', linewidth=2, label='Decision Boundary' if i == 1 else "")
    ...
    intercept_ratio = [b0 / b2 if b2 != 0 else None for b0, b2 in zip(beta0_list, beta2_list)]
